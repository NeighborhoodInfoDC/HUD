** PROGRAM NAME: CHAS_0610_sum_phase2.sas
**
** PROJECT:  DMPED - Affordable Housing Assessment 
**
** DESCRIPTION :  create summary files for Tables 1, 3, 8, and 16 for phase 2 (neighborhood cluster level) 
**
** ASSISTING PROGRAMS:
...
data test;
	set HUD.chas_sum_tr10 (keep = T8_est13_t T8_est26_t
T8_est39_t
T8_est52_t
T8_est65_t
);
...
data test2;
	set HUD.chas_2006_2010_dc (keep = T8_est13 T8_est26
T8_est39
T8_est52
T8_est65
);
run;
 ...
 %Create_summary_from_tracts( 
  ...
  tract_yr=2010,
  register=N, 
  creator_process=chas_0610_sum_phase2.sas,
  restrictions=,
  revisions=,
  mprint=y
);
...
data master_num;
	set hud.chas_sum_cltr00 (rename=(cluster_tr2000=geo2012)) city (rename=(city=geo2012));
run;
...
PROC EXPORT DATA= WORK.Table_cost_burden 
            OUTFILE= "D:\DCData\Libraries\HUD\&level\cost_burden_ph2.csv"
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= WORK.housing_needs 
            OUTFILE= "D:\DCData\Libraries\HUD\&level\housing_needs_ph2.csv"
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= WORK.elderly_income 
            OUTFILE= "D:\DCData\Libraries\HUD\&level\elderly_income_ph2.csv"
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= WORK.vacant_for_rent
            OUTFILE= "D:\DCData\Libraries\HUD\&level\vacant_for_rent_ph2.csv"
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= WORK.rent_occ
            OUTFILE= "D:\DCData\Libraries\HUD\&level\rent_occ_ph2.csv"
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
